...
- **storage / unlimitedStorage** — хранить настройки пользователя (выбранные сервисы, свой прокси/пул, тема) и кэш списка бесплатных прокси и реестра РКН локально.
...
- **host_permissions `<all_urls>`** — пользователь сам выбирает, какие сайты маршрутизировать; список доменов заранее неизвестен, поэтому нужен доступ ко всем URL для применения PAC-маршрутизации. Расширение НЕ читает и НЕ изменяет содержимое страниц.
...
## 6. Раскрытие данных (Data usage)

В форме Data usage отметить:
- **Не собираются** персональные данные, история, контент страниц.
- Локально хранятся только пользовательские настройки (не передаются никуда).
- Внешние сетевые запросы: загрузка публичных списков бесплатных прокси и реестра РКН (это данные конфигурации, не данные пользователя), и тестовые запросы для проверки живости прокси.
